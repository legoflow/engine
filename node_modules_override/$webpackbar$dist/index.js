...
class WebpackBarPlugin extends _webpack2.default.ProgressPlugin {
  ...
  apply(compiler) {
    ...
    this.options.once && (isStarted = true)
  }
    ...
    if (!wasRunning && isRunning) {
      ...
      if (this.options.minimal) {
        ...
        if (this.options.compiledIn) {
          _consola2.default.success(`compiled in ${(0, _prettyTime2.default)(time, 'ms')}`);
        }
      }
  ...
  render() {
    ...
    const stateLines = _lodash2.default.sortBy(Object.keys(sharedState), n => n).map(name => {
      ...
      return `${[(0, _utils.renderBar)(state.progress, state.color), ` ${ state.msg }`, ` (${ _chalk.green.bold(`${state.progress || 0}%`) })`, '\n' ,_chalk2.default.grey(state.details && state.details[0] || ''), ' ' ,_chalk2.default.grey(state.details && state.details[1] || '')].join('')}\n${state.request ? _chalk2.default.grey((0, _utils.elipsesLeft)((0, _utils.formatRequest)(state.request), columns - 2)) : ''}\n`;
    });

    if (hasRunning()) {
      const log = `${stateLines.join('\n')}`;
      this.logUpdate(log);
    }
  }
}
